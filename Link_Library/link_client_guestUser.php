...
if (!empty($_POST)) {
    if (isset($_POST['user_csv'])) {
        ...
        $target = "../guestCSVfiles/";
        $target1 = "/guestCSVfiles/";

        $fullcsvpath = $target1 . $upload_file_name;
//echo $fullcsvpath;
        $result1 = $obj->uploadGuestUserCsv($clientid, $user, $filename, $filtempname, $fullcsvpath);
...
//        echo '<pre>';print_r($_POST);die;
        ...
        $result = $obj->guestUserForm($clientid, $user, $first_name, $middle_name, $last_name, $emp_code, $dob, $fathername, $email_id, $designation, $department, $contact, $location, $branch, $grade, $gender,$companyname);
        print_r($result);
    }
} else {
    ?>
